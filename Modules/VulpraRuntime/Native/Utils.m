...
int spawnRoot(NSString *path, NSArray<NSString *> *args) {
    ...
    // #7: Use WNOHANG polling with a 10s timeout instead of blocking waitpid.
    // The old code blocked the attach queue indefinitely if ptrace_jit hung.
    int status = 0;
    const useconds_t pollInterval = 10000;  // 10ms
    const int maxPolls = 1000;              // 10s total
    for (int poll = 0; poll < maxPolls; poll++) {
        pid_t result = waitpid(taskPID, &status, WNOHANG);
        if (result == taskPID) break;
        if (result == -1) {
    ...
    // If still not exited after timeout, kill and return error.
