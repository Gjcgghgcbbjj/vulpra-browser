...
grep -Fq 'Tools/Runtime/prepare-runtime-headers.sh' "$build_app" || fail "archive does not stage platform-neutral runtime headers"
...
python3 - "$ROOT/Tools/Release/create-ipa.sh" <<'PY'
...
if any(position < 0 for position in positions) or positions != sorted(positions):
    raise SystemExit('FAIL: nested TrollStore signing order is incomplete or unsafe')
PY
