...
- Phase A ownership, artifact-v4, public-contract, ABI-boundary, and dormant
  Xcode-target contracts are present and portable-verified.
- The current staged runtime still uses the temporary `GeckoView`/Helper path.
- Atomic cutover is not ready until the v4 artifact, ABI inventory, independent
  process startup, App protocol migration, package update, simulator navigation,
  and physical-device/JIT evidence are complete.
