...
0 分 0 progress），四次真实 CI speedometer3 单次 gate 全绿：
...
| 31369430972 | e46c607 | 同上 | 2.108 | 171 247 | 0 | PASS |

进度 title 全程可见（`bar=0/11 → … → 9/11 → score=`），分数来自引擎 title 通道
`Engine title: VulpraBenchmark speedometer3 score=…`。证据已下载
`/tmp/bench-gate-31345381395/`（run 31345381395）、
`/tmp/bench-gate-31366323000-new/benchmark-gate/`（run 31366323000）与
`/tmp/bench-gate-31369430972/`（run 31369430972）。

**噪声结论**：四次运行代码实质相同（差异仅为 MotionMark 排除与 harness 演进），score = **2.435 / 1.809 / 0.97 / 2.108**
（中位数 **1.959**，极差 2.5 倍），耗时 68s→114s→205s→171s。Simulator + 解释器模式的单次 gate 噪声大：
1 次不足以作为稳定基线；4 次取中位数（**1.959**）作为**当前模拟器基线**，但仍属同一 engine 快照的相对回归代理，
...
## 真机对照结果（2026-08-10，Safari 与 Vulpra 首跑）

按 SOP 第 1、2 步，用户在 iPhone 上分别用 **Safari** 与 **Vulpra**（最新 IPA，e46c607，v5 r0.3 引擎）
打开官方 Speedometer 3.0（与 CI 同一 4-suite 子集、单次迭代）：

| 环境 | 分数 | 说明 |
| --- | --- | --- |
| Safari 真机（JIT 开） | **11.24** | 结果页截图 + 用户口报 |
| Vulpra 真机（内容进程解释器，JIT 关） | **5.267** | 最新 IPA（e46c607，v5 r0.3 引擎）；
  content 子进程自 `b749de8` 起真机 `JS::DisableJitBackend()`，benchmark JS 在解释器模式跑 |
| Simulator 中位数（JIT 关） | **1.959** | 2.435 / 1.809 / 0.97 / 2.108 四次中位数 |

机型 + iOS 版本待补（解读绝对值需要；当前只报比值）。

### 结论（首跑，单次样本）

1. **Vulpra 真机 ≈ Safari 的 47%**（5.267 / 11.24），同机同子集同迭代——这是"落后几倍"的第一个
   真机可比答案：**约落后 2.1 倍**。与真机体感（卡、发热、滑动慢半拍）方向一致。
2. **5.267 是解释器模式分数（口径更正）**：e46c607 IPA 的内容进程自 `b749de8` 起在真机调
   `JS::DisableJitBackend()`，benchmark JS 实际解释器运行——所以 5.267 与 Simulator 中位数 1.959 的
   ≈2.7 倍是**真机/模拟器硬件差**，不是 JIT 开关差。**JIT 开关差**已由 2026-08-11 的 Simulator JIT 开 run（31425316658，
   引擎 tag `vulpra-engine-v5-jit-sim-candidate`）实测并归档到下方"JIT 开关差量化"。
3. **口径说明**：三者均为**单次**样本，SOP 要求至少 3 次取中位数后才算正式基线；Simulator 侧单次极差
   2.5 倍，真机单次也可能有类似噪声，2.1 倍落后是方向性结论，不是精确值。

### JIT 开关差量化（2026-08-11 归档）

Simulator 上同引擎（v5 248 补丁）同子集同机型的 JIT 开/关对比——**纯 JIT 开关差，
排除真机/模拟器硬件差**：

| 环境 | scoreMedian | 说明 |
| --- | --- | --- |
| Simulator（JIT 关，解释器） | **1.959** | 2.435 / 1.809 / 0.97 / 2.108 四次中位数 |
| Simulator（JIT 开） | **3.451** | run 31425316658，`vulpra-engine-v5-jit-sim-candidate`，单次 |
| **开关差** | **≈1.76×（+76%）** | 3.451 / 1.959 |

- 结论：**同硬件同引擎下 JIT 开关差约 1.76×**。Simulator 的 JIT 天然开
  （`#if !TARGET_OS_SIMULATOR` 不生效），此差值不依赖越狱/调试器，是引擎本身在
  Ion/Baseline vs 解释器之间的纯编译后端差距。
- 对真机含义（方向性）：真机 5.267 是解释器模式；若真机 MAP_JIT 可用（越狱 Allow JIT /
  debugserver 附加路线），同引擎在真机上理论提升空间同比例（≈1.7×，潜在 ~9 分），
  但仍需真机实测——真机还叠加了 MAP_JIT 是否真正成功、witness API 路径、App 侧改动等
  变量，不能把 Simulator 差值直接外推为真机收益。
- 口径：Simulator 单次噪声大（历史极差 2.5×），3.451 为单次样本，方向性结论而非精确值。

### 待办

1. （可选，加分）Safari / Vulpra 各再跑 2 次取中位数，确认 2.1 倍落后区间
2. 补：iPhone 型号 + iOS 版本（归档用）
3. 后续性能优化以 Vulpra 真机 5.267 → 追 Safari 11.24 为可度量目标；模拟器 gate 只作回归代理
