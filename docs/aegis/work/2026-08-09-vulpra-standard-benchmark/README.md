...
| **Vulpra 真机（JIT 开）** | **5.267** | 最新 IPA（e46c607，v5 r0.3 引擎） |
...
2. **JIT 确实是大头**：Vulpra 真机（JIT 开）是模拟器中位数（JIT 关）的 **≈2.7 倍**（5.267 / 1.959）；
   Safari 则是模拟器的 ≈5.7 倍。解释器→JIT 的量级差距被实测证实，之前"模拟器分数不可外推真机"的判断成立。
