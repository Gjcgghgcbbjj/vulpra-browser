...
2. **5.267 是解释器模式分数（口径更正）**：e46c607 IPA 的内容进程自 `b749de8` 起在真机调
   ...
   ≈2.7 倍是**真机/模拟器硬件差**，不是 JIT 开关差。**JIT 开关差**由 run 31374470622（Simulator JIT 开）
   对比 1.959（Simulator JIT 关）量化，编译完成后归档到本节；在那之前"JIT 是性能大头"只有 Safari
   对照（11.24）作方向性支撑，不是 Vulpra 同引擎的实测差。
