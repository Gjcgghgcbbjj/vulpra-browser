...
if ! git -C "$SUBMODULE_PATH" rev-parse -q --verify "$RELEASE_TAG^{commit}" >/dev/null 2>&1; then
	...
	echo "Run Tools/Gecko/update-gecko.sh to fetch and checkout the release tag."
	exit 1
fi
...
if [[ "$HEAD_COMMIT" != "$PINNED_COMMIT" ]]; then
	...
	echo "Run Tools/Gecko/update-gecko.sh to sync the submodule commit before applying patches."
	exit 1
fi
