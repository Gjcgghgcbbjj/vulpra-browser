...
final class BrowserChromeView: UIView, UITextFieldDelegate {
    ...
    private let toolsButton = PressableButton(symbol: "ellipsis.circle", accessibilityLabel: VulpraL10n.text("page_tools.title"))
    ...
    private func configure() {
        ...
        [backButton, forwardButton, reloadButton, toolsButton, tabsButton].forEach(buttonRow.addArrangedSubview)
        ...
        toolsButton.addTarget(self, action: #selector(showPageTools), for: .touchUpInside)
    ...
    @objc private func showPageTools() { delegate?.browserChromeDidRequestPageTools(self) }
