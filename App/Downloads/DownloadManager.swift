...
final class DownloadManager {
    ...
    var onChange: (() -> Void)?
    // #3: Debounce disk writes during active downloads. The old code called
    // persist() on every progress callback (hundreds of times for a large file).
    ...
    func update(path: String, bytes: Int64) -> Bool {
        ...
        // #3: Don't persist on every progress tick — debounce the write.
        schedulePersist()
    ...
    /// #3: Debounced persist — coalesces rapid progress callbacks into one write.
