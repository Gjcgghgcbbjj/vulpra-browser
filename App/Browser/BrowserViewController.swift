...
final class BrowserViewController: UIViewController, BrowserChromeViewDelegate, TabManagerDelegate,
    ...
    override func didReceiveMemoryWarning() {
        ...
        tabManager.releaseMemoryPressure()
    }
    ...
    private func showFailure(_ message: String) {
        ...
        if failureLabel?.text == message { return }
        clearVisibleContent()
    ...
    func tabManagerDidChange(_ manager: TabManager) {
        showSelectedTab()
        let activeIDs = Set(manager.tabs.map(\.id))
        recordedURLs = recordedURLs.filter { activeIDs.contains($0.key) }
        guard let tab = manager.selectedTab, let url = tab.url, !tab.isLoading, tab.progress == 100,
