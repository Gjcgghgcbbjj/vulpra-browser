...
protocol BrowserTabObserver: AnyObject {
    ...
    func browserTabSessionDidChange(_ tab: BrowserTab)
...
final class BrowserTab: NavigationDelegate, ProgressDelegate, ContentDelegate {
    ...
    func activate(settings: BrowserSettings, windowID: String? = nil) -> GeckoSession {
        ...
        session = created
        if windowID == nil, pendingLoadURL == nil {
            pendingLoadURL = url
        }
        ...
        if created.isOpen() {
            ...
            GeckoEngineGate.whenReady { [weak self] in
                ...
                self.flushPendingLoadIfPossible()
                self.observer?.browserTabSessionDidChange(self)
            }
        }
        return created
    }

    private func flushPendingLoadIfPossible() {
        guard let session, session.isOpen(), let target = pendingLoadURL else { return }
        pendingLoadURL = nil
    ...
    func onCanGoBack(session: GeckoSession, canGoBack: Bool) {
        ...
        observer?.browserTabPresentationDidChange(self)
    }
    ...
    func onCrash(session: GeckoSession) { suspend(); observer?.browserTabSessionDidChange(self) }
    ...
    func onExternalResponse(session: GeckoSession, response: ExternalResponseInfo) async -> Bool {
        ...
        await dispatchDownload(response)
    }

    @MainActor private func dispatchDownload(_ response: ExternalResponseInfo) async -> Bool {
