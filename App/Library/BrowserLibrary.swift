...
final class HistoryStore {
    ...
    func matches(_ query: String, limit: Int) -> [HistoryVisit] {
        ...
        matches.reserveCapacity(limit)
        // #8: Only scan the most recent 500 visits (not all 5000). For omnibox
        // suggestions, recency is a better signal than exhaustive search — users
        // rarely search for something they visited months ago.
        let scanLimit = min(500, visits.count)
        for visit in visits.prefix(scanLimit) {
